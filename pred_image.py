...
from models.display import draw_target_in_image, pil_to_cv, imwrite
import torch
...
# pred
model = efficientdet_d0(True, image_size=1920).to(device)
model.eval()
with torch.no_grad():
    target = model([image], score_thresh=0.2, nms_thresh=0.2)[0]
